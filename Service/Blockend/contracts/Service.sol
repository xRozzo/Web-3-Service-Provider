...
contract Service {
    ...
    */
    ...
    function RequestService(uint256 _index, uint256 _EthPriceInWey) public payable {
        ...
        // with this you make a copy of the service in the memory but not inteeract with the state in general
        // so good to check data but no use if have to change the provider
        ...
        // for changing the state of the blockchain you need to tap to the actual object
        serviceAvailable[_index].state = State.REQUESTED; // prevent the reentrancy
    ...
    */
    ...
    function ProviderRevokeService(uint256 _index) public OnlyValidatedProviders() {
        ...
        address _user = serviceAvailable[_index].provider;
        if (_provider != msg.sender) {
            revert Not__Correct__Provider();
        }
        delete UserToProviderService[_user][msg.sender];
    ...
    */
    ...
    function ChangeToShipping(uint256 _index) public OnlyValidatedProviders {
        ...
        if (serviceAvailable[_index].state != State.REQUESTED) {
            revert Not__Correct__State();
        }
    ...
    */
    ...
    function ChangeToShipped(uint256 _index) public OnlyValidatedProviders {
        ...
        if (serviceAvailable[_index].state != State.SHIPPING) {
            revert Not__Correct__State();
        }
    ...
    function getUserServiceExist(address _user, address _provider) public view returns(bool) {
        return(UserToProviderServiceExist[_user][_provider]);
    }
}
